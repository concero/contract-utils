...
function getClients(viemChain, url, account = (0, import_accounts.privateKeyToAccount)(
  ...
  const publicClient = (0, import_viem3.createPublicClient)({ transport: (0, import_viem3.http)(url), chain: viemChain });
  const walletClient = (0, import_viem3.createWalletClient)({ transport: (0, import_viem3.http)(url), chain: viemChain, account });
...
function getFallbackClients(chain, account) {
  ...
  const walletClient = (0, import_viem3.createWalletClient)({ transport, chain: viemChain, account });
  return { walletClient, publicClient, account };
}
...
var TokenSender = class {
  ...
  async sendToken(tokenAddress, amount, recipient) {
    ...
    console.log(
      `Current ${tokenSymbol} balance ${(0, import_viem4.formatUnits)(tokenBalance, tokenDecimals)}`
    );
    if ((0, import_viem4.parseUnits)(amount, tokenDecimals) > tokenBalance) {
      throw new Error(
        `Insufficient balance. Required: ${amount}, available: ${(0, import_viem4.formatUnits)(tokenBalance, tokenDecimals)}`
      );
    }
    const txHash = await this.walletClient.writeContract({
      ...
      args: [recipient, (0, import_viem4.parseUnits)(amount, tokenDecimals)],
